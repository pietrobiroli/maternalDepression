...
*stepdownB reg (hap1 sat1) treat spillover if ~purecontrol, options(r cluster(village)) iter(100) txcontrols(educ_baseline) rcluster(village)
capture program drop stepdownB
program define stepdownB, rclass
	gettoken cmd 0 : 0
...
quietly {
	...
	*assign the same randomization value to all of the obs in rcluster
	...
	foreach lhsvar of numlist 1/`numvars' {
		...
		`cmd' `depvar' `simtreatment' `varlist' `controlvars' `txcontrolvars' `if' `in' `weights', `options'
